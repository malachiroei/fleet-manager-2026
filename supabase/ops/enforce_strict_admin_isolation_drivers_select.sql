-- Emergency hardening: strict driver visibility isolation between peer admins.
...
-- 2) Remove ALL existing SELECT policies on drivers (including legacy/unknown names).
DO $$
DECLARE p record;
BEGIN
  FOR p IN
    ...
    WHERE schemaname = 'public'
      ...
      AND cmd = 'SELECT'
  LOOP
    EXECUTE format('DROP POLICY IF EXISTS %I ON public.drivers', p.policyname);
  END LOOP;
END $$;
