...
function VehicleComplianceDocSubtext({ v }: { v: Vehicle }) {
  ...
  if (isVehicleExemptFromAnnualTestNow(v)) {
    rows.push({
      key: 'texempt',
      className: 'text-emerald-900 dark:text-emerald-200/95',
      children: <>טסט: פטור בשנה הראשונה (נדרשת אגרת רישיון בלבד)</>,
    });
  } else if (t === 'expired') {
    rows.push({
      key: 'te',
      className: 'text-red-800 dark:text-red-200/95',
      children: (
        <>
    ...
    rows.push({
      key: 'tw',
      className: 'text-amber-900 dark:text-amber-200/95',
      children: (
        <>
          טסט: לטיפול
  ...
  if (!docNonOk && v.status === 'expired') {
    rows.push({
      key: 'syse',
      className: 'text-red-800 dark:text-red-200/90',
...
function vehicleCompliancePill(v: Vehicle) {
  if (!v.is_active) {
    return (
      <span
        className="inline-flex items-center gap-1 rounded-full border border-slate-300 bg-slate-100 px-2.5 py-0.5 text-[11px] font-semibold text-slate-700 dark:border-white/15 dark:bg-slate-700/80 dark:text-slate-200"
...
function renderVehicleHudOptionalCell(colId: string, v: Vehicle, driverName: string): ReactNode {
  switch (colId) {
    ...
    case 'assigned_driver':
      return (
        <span className="block truncate text-sm text-slate-700 dark:text-slate-300" title={driverName || undefined}>
          {driverName || '—'}
        </span>
      );
    case 'compliance':
    ...
    case 'ownership': {
      ...
      return (
        <span className="block truncate text-sm text-slate-700 dark:text-slate-300" title={ownershipLabel || undefined}>
          {ownershipLabel || '—'}
        </span>
      );
    }
    case 'odometer':
      return (
        <span className="font-mono text-sm tabular-nums text-slate-800 dark:text-slate-200" dir="ltr">
          {v.current_odometer.toLocaleString('he-IL')}
        </span>
      );
    case 'is_active':
      return <span className="text-sm text-slate-700 dark:text-slate-300">{v.is_active ? 'פעיל' : 'לא פעיל'}</span>;
    case 'test_expiry':
      return (
    ...
    case 'monthly_total_cost':
      return (
        <span dir="ltr" className="font-mono text-sm text-slate-800 dark:text-slate-200">
    ...
    case 'year':
      return <span className="text-sm text-slate-800 dark:text-slate-200">{v.year != null ? String(v.year) : '—'}</span>;
    case 'road_ascent':
      return (
        <span className="text-sm text-slate-800 dark:text-slate-200" dir="ltr">
          {vehicleRoadAscentLabel(v)}
        </span>
      );
    case 'manufacturer':
      return <span className="block truncate text-sm text-slate-700 dark:text-slate-300">{fmtCellText(v.manufacturer)}</span>;
    case 'model':
      return <span className="block truncate text-sm text-slate-700 dark:text-slate-300">{fmtCellText(v.model)}</span>;
    case 'color':
      return <span className="block truncate text-sm text-slate-700 dark:text-slate-300">{fmtCellText(v.color)}</span>;
    case 'fuel_type':
      return <span className="block truncate text-sm text-slate-700 dark:text-slate-300">{fmtCellText(v.fuel_type)}</span>;
    case 'group_name':
      return <span className="block truncate text-sm text-slate-700 dark:text-slate-300">{fmtCellText(v.group_name)}</span>;
    case 'chassis_number':
      return (
    ...
    case 'safety_officer':
      return <span className="block truncate text-sm text-slate-700 dark:text-slate-300">{fmtCellText(v.safety_officer)}</span>;
    case 'vat_recognized':
      return (
        <span className="font-mono text-sm text-slate-800 dark:text-slate-200" dir="ltr">
    ...
    case 'vehicle_standard':
      return <span className="block truncate text-sm text-slate-700 dark:text-slate-300">{fmtCellText(v.vehicle_standard)}</span>;
    case 'leasing_company_name':
      return <span className="block truncate text-sm text-slate-700 dark:text-slate-300">{fmtCellText(v.leasing_company_name)}</span>;
    ...
    case 'engine_volume':
      return <span className="text-sm text-slate-700 dark:text-slate-300">{fmtCellText(v.engine_volume)}</span>;
    case 'ignition_code':
      return (
        <span className="font-mono text-sm text-slate-800 dark:text-slate-200" dir="ltr">
    ...
    case 'tax_year':
      return <span className="text-sm text-slate-800 dark:text-slate-200">{v.tax_year != null ? String(v.tax_year) : '—'}</span>;
    case 'adjusted_price':
      return (
        <span dir="ltr" className="font-mono text-sm text-slate-800 dark:text-slate-200">
...
export function VehiclesHudTable({
  ...
  return (
    <div className="w-full max-w-[100vw] space-y-4 overflow-x-hidden sm:space-y-5">
      <div className="grid grid-cols-2 gap-2 rounded-xl border border-slate-200 bg-slate-100 p-3 shadow-sm sm:grid-cols-3 lg:grid-cols-5 sm:gap-3 sm:p-4 dark:border-cyan-500/20 dark:bg-[#0a1528]/90 dark:shadow-[0_0_24px_rgba(6,182,212,0.08)]">
        <StatCardButton
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11px] font-medium leading-snug text-slate-600 sm:text-xs dark:text-slate-400">סה״כ רכבים</p>
          </div>
        </StatCardButton>
        <StatCardButton
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11px] font-medium leading-snug text-amber-900/90 sm:text-xs dark:text-amber-100/90">
              טסט / ביטוח
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11px] font-medium leading-snug text-red-800/95 sm:text-xs dark:text-red-100/90">
              טסט / ביטוח
              <span className="text-slate-500 dark:text-slate-400"> — </span>
              פג תוקף
            </p>
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11px] font-medium leading-snug text-slate-600 sm:text-xs dark:text-slate-400">ללא נהג משויך</p>
          </div>
        </StatCardButton>
      </div>
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between sm:gap-3 sm:p-4 dark:border-white/10 dark:bg-slate-950/80">
        <div className="min-w-0 sm:max-w-xs sm:flex-1">
          <label className="mb-1 block text-[11px] font-medium text-slate-600 dark:text-slate-400">חיפוש</label>
          <Input
            placeholder="חפש רכב…"
            value={search}
            onChange={(e) => onSearchChange(e.target.value)}
            className="h-10 border border-input bg-background text-foreground placeholder:text-muted-foreground dark:border-white/10 dark:bg-black/30 dark:text-white dark:placeholder:text-slate-500"
          />
        </div>
        ...
        <div className="w-full min-w-[8rem] sm:w-40 sm:shrink-0">
          <label className="mb-1 block text-[11px] font-medium text-slate-600 dark:text-slate-400">בעלות</label>
        ...
        <div className="w-full min-w-[8rem] sm:w-40 sm:shrink-0">
          <label className="mb-1 block text-[11px] font-medium text-slate-600 dark:text-slate-400">קבוצה</label>
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-cyan-500/20 dark:bg-[#070d18]/95 dark:shadow-[0_0_32px_rgba(6,182,212,0.06)]">
          ...
          >
            <TableHeader>
              <TableRow className="border-slate-200 bg-slate-100 hover:bg-slate-100 dark:border-cyan-500/15 dark:bg-black/40 dark:hover:bg-black/40">
                ...
                <TableHead className="p-0 px-3 py-2.5 text-right align-middle text-xs font-semibold text-slate-700 whitespace-nowrap dark:text-slate-300">
                  מספר רישוי
                </TableHead>
            ...
            <TableBody>
              {pageSlice.length === 0 ? (
                <TableRow className="border-0 hover:bg-transparent">
                  <TableCell colSpan={vehicleTableColSpan} className="py-16 text-center text-slate-500 dark:text-slate-400">
                ...
                pageSlice.map((v) => {
                  ...
                  return (
                    <TableRow
                      ...
                      className={cn(
                        'border-slate-100 transition-all duration-200 dark:border-white/5',
                    ...
                    >
                      ...
                      <TableCell
                        className="p-0 px-3 py-2.5 align-middle font-mono text-sm font-medium tabular-nums text-sky-700 dark:text-cyan-200/90"
                        dir="ltr"
                      >
                        <Link to={`/vehicles/${v.id}`} className="text-sky-800 hover:underline dark:text-inherit">
                          {v.plate_number}
                        </Link>
                      </TableCell>
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button
                              ...
                              variant="ghost"
                              size="icon"
                              className="h-8 w-8 text-slate-500 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-white/10 dark:hover:text-cyan-200"
                              aria-label="פעולות"
                            >
