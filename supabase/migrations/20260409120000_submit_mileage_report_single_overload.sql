-- =============================================================================
-- PostgREST: "Could not choose the best candidate function" כשיש שני עומסים —
-- למשל submit_mileage_report(uuid, integer, text) וגם (uuid, numeric, text).
-- JSON שולח מספר שלם → PostgreSQL יכול להתאים לשניהם. נשארים רק עם numeric.
-- =============================================================================
...
BEGIN
  ...
  LOOP
    sig := r.identity_args;
    IF sig IS DISTINCT FROM 'uuid, numeric, text' THEN
      EXECUTE format('DROP FUNCTION IF EXISTS public.submit_mileage_report(%s)', sig);
    END IF;
  END LOOP;
END $$;

-- ודא שהגרסה הנכונה קיימת (אם נמחקה בטעות — הרץ מחדש מיגרציות 061400/081000)
